...
bool Renderer::Initialize(Window* window, Graphics* graphics)
{
    ...
    if FAILED(graphics->d3dDevice->CreateVertexShader(vShader->GetBufferPointer(), vShader->GetBufferSize(), NULL, &vertexShader))
        return false;
    ...
    // cria o input layout
    if FAILED(graphics->d3dDevice->CreateInputLayout(layoutDesc, 3, vShader->GetBufferPointer(), vShader->GetBufferSize(), &inputLayout))
        return false;
    ...
    // cria o pixel shader
    if FAILED(graphics->d3dDevice->CreatePixelShader(pShader->GetBufferPointer(), pShader->GetBufferSize(), NULL, &pixelShader))
        return false;
    ...
    if FAILED(graphics->d3dDevice->CreateRasterizerState(&rasterDesc, &rasterState))
        return false;
    ...
    if FAILED(graphics->d3dDevice->CreateBuffer(&vertexBufferDesc, nullptr, &vertexBuffer))
        return false;
    ...
    if FAILED(graphics->d3dDevice->CreateBuffer(&indexBufferDesc, &indexData, &indexBuffer))
        return false;
    ...
    if FAILED(graphics->d3dDevice->CreateBuffer(&constBufferDesc, &constantData, &constantBuffer))
        return false;
    ...
    samplerDesc.MaxAnisotropy = (graphics->d3dDevice->GetFeatureLevel() > D3D_FEATURE_LEVEL_9_1) ? 16 : 2;
    ...
    if FAILED(graphics->d3dDevice->CreateSamplerState(&samplerDesc, &sampler))
        return false;
    ...
    uint vertexOffset = 0;
    graphics->d3dDeviceContext->IASetVertexBuffers(0, 1, &vertexBuffer, &vertexStride, &vertexOffset);
    graphics->d3dDeviceContext->IASetIndexBuffer(indexBuffer, DXGI_FORMAT_R16_UINT, 0);
    graphics->d3dDeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    graphics->d3dDeviceContext->IASetInputLayout(inputLayout);
    graphics->d3dDeviceContext->VSSetShader(vertexShader, NULL, 0);
    graphics->d3dDeviceContext->VSSetConstantBuffers(0, 1, &constantBuffer);
    graphics->d3dDeviceContext->PSSetShader(pixelShader, NULL, 0);
    graphics->d3dDeviceContext->PSSetSamplers(0, 1, &sampler);
    graphics->d3dDeviceContext->RSSetState(rasterState);
...
void Renderer::RenderBatch(ID3D11ShaderResourceView* texture, SpriteData** sprites, uint cont)
{
    ...
    Graphics::d3dDeviceContext->PSSetShaderResources(0, 1, &texture);
    ...
    while (cont > 0)
    {
        ...
        Graphics::d3dDeviceContext->Map(vertexBuffer, 0, mapType, 0, &mappedBuffer);
        ...
        Graphics::d3dDeviceContext->Unmap(vertexBuffer, 0);

        // desenha sprites 
        uint startIndex = (uint)vertexBufferPosition * IndicesPerSprite;
        uint indexCount = (uint)batchSize * IndicesPerSprite;
        Graphics::d3dDeviceContext->DrawIndexed(indexCount, startIndex, 0);
